...
7. **Mesh Splitting, Mouth Integration & UDIM Sequencing:** Non-destructively separates the head mesh from the body mesh. It then fully integrates the separated Genesis 9 Mouth mesh (purging shape keys, merging teeth materials, and joining geometry). Finally, it executes a mathematical UV sequencer that dynamically arranges all meshes into a flawless UDIM layout (Torso: 1001, Legs/Mouth: 1002, Arms/Nails: 1003) and runs a `numpy` shape key cleaner that instantly purges morph targets with zero geometric effect on the separated meshes.
